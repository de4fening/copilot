...
classes = ['Ped', 'bicycle', 'car', 'motorbike', 'aeroplane', 'bus', \
...
class OBSTACLE(BoundBox):
    ...
    def update_obstacle(self, dst, fps, n=5):
        ...
        if (self.__count > self.position_hist.maxlen):
            self.velocity = ((self.position - self.position_hist[0]) * fps / self.position_hist.maxlen * 5 / 18).astype(
                int)
        self.__count += 1
        self.position_hist.append(dst)
...
class LANE_HISTORY:
    def __init__(self, fps, queue_depth=12, poly_col=np.array([1, 1, 1]), test_points=[300, 500, 700],
                 poly_max_deviation_distance=50, smoothing=10, ploty=np.array([])):
        self.fps = fps
    ...
    def compute_curvature(self, alpha, beta):
        ...
        self.curvature = int(((beta ** 2 + (2 * lp[0] * y_eval * alpha ** 2 + \
                                            lp[1] * alpha) ** 2) ** 1.5) / (
                                 np.absolute(2 * lp[0] * (alpha * beta) ** 2)))
        return
...
class LANE_DETECTION:
    ...
    def __init__(self, img, fps,
        ...
        self.ploty = np.linspace(int(self.UNWARPED_SIZE[1] * 0.45), self.UNWARPED_SIZE[1] - 1,
                                 int(self.UNWARPED_SIZE[1] * 0.4), dtype=int)
    ...
    def draw_lane_curvature_text(self, img):
        ...
        cv2.putText(img, txt_values, (offset_x, offset_y + self._pip__y_offset * 2), self.font, sz, BLACK, 2,
                    cv2.LINE_AA)
    ...
    def combine_images(self, lane_area_img, lines_img, lane_hotspots_img):
        ...
        lane_area_img[self._pip__y_offset: self._pip__y_offset + self._pip_size[1],
        self._pip__x_offset: self._pip__x_offset + self._pip_size[0]] = small_lines
        start_offset_y = self._pip__y_offset
        start_offset_x = 2 * self._pip__x_offset + 1 * self._pip_size[0]
        lane_area_img[start_offset_y: start_offset_y + self._pip_size[1],
        start_offset_x: start_offset_x + self._pip_size[0]] = small_hotspots
